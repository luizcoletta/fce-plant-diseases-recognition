function [bestmem,bestval,nfeval,valEvol,FEvol,CREvol] = optD2E(opar,fname,VTR,D,XVmin,XVmax,NP,itermax,strategy,stm,ft,nameFile,rD2E,vF,vCR)
% *************************************************************************
% D2E:  DE extension based on Storn's code
%       (http://www.icsi.berkeley.edu/~storn/code.html) - see more details
%       below.
%                       
% Example:  [best,f,nf,valEvol,FEvol,CREvol] = D2E(opar,'objFunc_D2E',
%                                                  VTR,D,XVmin,XVmax,NP,
%                                                  itermax,strategy,st,
%                                                  time,nameFile,1,vF,vCR);
%
% Author: Luiz F. S. Coletta (luiz.fersc@gmail.com) - 10/04/13
% Update: Luiz F. S. Coletta - 01/11/13
% *************************************************************************
...
%       global minimum is expected. D0E is also somewhat sensitive to
...
% 	XVmin = [-2 -2];
% 	XVmax = [2 2];
% 	[bestmem,bestval,nfeval] = devec3('rosenbrock',VTR,D,XVmin,XVmax);
...
if ((vCR < 0) || (vCR > 1))
     vCR=0.9;
     fprintf(1,'CR should be from interval [0,1]; set to default value 0.9\n');
end
if ((vF < 0.1) || (vF > 1))
     vF=0.5;
     fprintf(1,'F should be from interval [0.1,1]; set to default value 0.5\n');
end
if (itermax <= 0)
    itermax = 200;
...
for i=1:NP
    nx = roundn(XVmin(1)+(XVmax(1)-XVmin(1)).*rand,-3);
    ny = round(XVmin(2)+(XVmax(2)-XVmin(2)).*rand);
    pop(i,:) = [nx, ny];
end
...
val(1)  = feval(fname,[F(i),CR(i),strategy,opar], pop(i,:), nameFile);
bestval = val(1);                 % best objective function value so far
nfeval  = nfeval + 1;
for i=2:NP                        % check the remaining members
    val(i) = feval(fname,[F(i),CR(i),strategy,opar], pop(i,:), nameFile);
    nfeval  = nfeval + 1;
...
while ((iter < itermax) && (bestval > VTR))
    ...
    for i=1:NP
        mui(i,:) = rand(1,D) < CR(i); % all random numbers < CR are 1, 0 otherwise
    end 
    ...
    if (st == 1)                      % DE/best/1
        ...
        ui = bm + repmat(F(ibest),1,D).*(pm1 - pm2);
    elseif (st == 2)                  % DE/rand/1
        %ui = pm3 + F*(pm1 - pm2);    % differential variation
        ui = pm3 + repmat(F(a3),1,D).*(pm1 - pm2);
    elseif (st == 3)                  % DE/rand-to-best/1
        %ui = popold + F*(bm-popold) + F*(pm1 - pm2);
        ui = popold + repmat(F,1,D).*(bm-popold) + repmat(F,1,D).*(pm1 - pm2);
    elseif (st == 4)                  % DE/best/2
        %ui = bm + F*(pm1 - pm2 + pm3 - pm4);  % differential variation
        ui = bm + repmat(F(ibest),1,D).*(pm1 - pm2 + pm3 - pm4);
    elseif (st == 5)                  % DE/rand/2
        %ui = pm5 + F*(pm1 - pm2 + pm3 - pm4);  % differential variation
        ui = pm5 + repmat(F(a5),1,D).*(pm1 - pm2 + pm3 - pm4);
    end
    
    % restricting within limits
    ui(:,1) = roundn(ui(:,1),-3);
    ui(:,2) = round(ui(:,2));
    ui(ui(:,1)<XVmin(1),1) = XVmin(1);
    ui(ui(:,1)>XVmax(1),1) = XVmax(1);
    ui(ui(:,2)<XVmin(2),2) = XVmin(2);
    ui(ui(:,2)>XVmax(2),2) = XVmax(2);
    ...
    for i=1:NP
        ...
        tempval = feval(fname,[F(i),CR(i),strategy,opar], pop(i,:), nameFile);
        nfeval  = nfeval + 1;
    ...
    % code extension for D2E algorithm
    % ---------------------------------------------
    valEvol = [valEvol, val'];
    propVal = (sum(valEvol)/100)/NP;
    diffPV = diff(propVal);
    vEvol = abs(diffPV(iter));
    
    fprintf('\n\n*****************************************************************\n');
    fprintf('F -> %1.4f - CR -> %1.4f \n', F(1), CR(1));
    fprintf('- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n\n');
  
    if ((vEvol == 0) && (rD2E == 1))
        nF = roundn(FMin+(FMax-FMin).*rand,-3);
        nCR = roundn(CRMin+(CRMax-CRMin).*rand,-3);
        F = nF * ones(NP,1);
        CR = nCR * ones(NP,1);
    end     
    ...
    if (t1 >= ft)
        disp(' ');
        disp('------ Time elapsed man! ------');
        disp(datestr(datenum(0,0,0,0,0,t1),'HH:MM:SS'));
        disp('-------------------------------');
        disp(' ');
        break;
    end
